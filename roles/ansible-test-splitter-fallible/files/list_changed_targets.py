...
class ElGrandeSeparator:
    ...
    def output(self, changes):
        ...
        for r in rels:
            for c in self.collections:
                job_prefix = (
                    f"integration-fallible-{c.collection_name()}"
                    if len(r) == 0
                    else f"integration-fallible-{c.collection_name()}-{r}"
                )
    ...
    def build_result_struct(self, batches):
        result = {
            "data": {
                "zuul": {"child_jobs": []},
                "child": {"fallible_targets": {}},
            }
        }

        for job, targets in batches:
            result["data"]["zuul"]["child_jobs"].append(job)
            result["data"]["child"]["fallible_targets"][job] = " ".join(targets)
        return result
